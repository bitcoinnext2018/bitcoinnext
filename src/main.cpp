...
#if defined(NDEBUG)
# error "BitCoinNext cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x44395fd0c30bf082fd3a4c3829b75425ad1cb52883360d24b482c7e4f54d7bb5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BitCoinNext: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BitCoinNext Signed Message:\n";
...
{
    // BitCoinNext: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BitCoinNext: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BitCoinNext: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1513754723, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Its a revolution of 2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042b3d2a4a8fcbe314566c67635dcfa01a11d01bf62431a5720f566c49bdfa8c722e27e9ef3fac5340cb0cfec4ca9371dfaef9016415056a5ed3ccd1d9b82fbd3f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1513754723;
        ...
        assert(block.hashMerkleRoot == uint256("0x8a1b76236338f9cfa9acbaf82495456d10b49eeaa058aa28bfbe0f77c8dcf076"));
...
unsigned char pchMessageStart[4] = { 0x2e, 0xe4, 0x66, 0x39 }; // BitCoinNext: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitCoinNextMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitCoinNextMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitCoinNextMiner(CWallet *pwallet)
{
    printf("BitCoinNextMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinnext-miner");
    ...
    try { loop {
        ...
        printf("Running BitCoinNextMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitCoinNextMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitCoinNextMiner, pwallet));
}
